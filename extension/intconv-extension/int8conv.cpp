...
torch::Tensor cu_int8_conv(torch::Tensor input, 
        torch::Tensor weight,
        int32_t stride,
        int32_t padding,
        int32_t dilation){
    CHECK_INPUT(input);
    CHECK_INPUT(weight);
    return int8_conv(input, weight, stride, padding, dilation);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("cu_int8_conv", &cu_int8_conv, "int8 convolution forward Nvidia GPU tensor core");
